...
const EditProduct = () => {
  const id = useRef(window.location.pathname.split("/").pop()).current;
  ...
  useEffect(() => {
    axios.get("/api/user/getsingleproduct/" + id).then((res) => {
      const product = res.data;
      setName(product.name);
      setDescription(product.description);
      setPrice(product.price);
      setDiscount(product.discount);
      setSelectedSections(product.categories);
  ...
  const handleImgChange = async (e) => {
    ...
    setImages((prev) => [...prev, ...compressedFiles]); // إضافة للصور الموجودة
  };
  ...
  const handleEditProduct = async (e) => {
    ...
    // ✅ التعديل: التمييز بين الصور الجديدة والقديمة
    // السبب: الصور القديمة لا يجب أن ترسل كملفات Multer
    images.forEach((img) => {
      if (img instanceof File) {
        formData.append("images", img); // الصور الجديدة
      } else if (typeof img === "object" && img.secure_url) {
        formData.append("oldImages", img.secure_url); // الصور القديمة من DB
      } else if (typeof img === "string") {
        formData.append("oldImages", img); // رابط قديم
      }
    });
    ...
    try {
      ...
      const res = await axios.post("/api/user/editproduct/" + id, formData, {
        headers: {
